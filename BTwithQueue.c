...
LINKED_TREE_NODE linked_tree_node_init(int key, void *data) {
	...
	LINKED_QUEUE linked_queue = (LINKED_QUEUE)malloc(sizeof(LINKED_QUEUE_NODE_t));
	linked_queue -> head = NULL;
	linked_queue -> tail = NULL;
	
	linked_queue_enqueue((linked_queue),data);
